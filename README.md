...
Runs 238 checks over the whole stack — XML parsing, chunking, MeSH extraction,
...
[20] Dev-server stability                ...  8 passed
[21] Static assets                       ... 12 passed
  238 passed, 0 failed
```
...
**The engine badge says "offline", or searches fail at random.**

Almost always Flask's auto-reloader. It watches every module in `sys.modules`,
and `sentence-transformers` imports several hundred `transformers` submodules
**lazily** — after the server is already serving. The reloader reads those as
"files changed" and restarts the process mid-request, reloading the embedding
model each time. The log gives it away:

```
Detected change in '...\site-packages\transformers\models\siglip\modeling_siglip.py', reloading
Detected change in '...\site-packages\transformers\models\whisper\modeling_whisper.py', reloading
 * Restarting with watchdog (windowsapi)
```

The reloader is **off by default** (`USE_RELOADER=false`), which removes the
problem. If you turn it on to edit code, `app.py` passes `exclude_patterns` so
installed packages are ignored and only project files are watched.

The badge is also no longer alarmist: it shows `engine: starting…` while the
model loads, retries with backoff, and only says `offline` after several
failures — then keeps retrying, and re-probes when you focus the tab.

**The first search is slow.** The embedding model now loads in a background
thread at startup (`PRELOAD_EMBEDDER=true`) rather than inside the first
request. Watch for `Embedding backend ready:` in the log.

**"sentence-transformers unavailable … Falling back to TF-IDF+SVD".** The model
could not be downloaded from Hugging Face. The app still works — with weaker
semantic matching — and the badge shows `tfidf-svd`. Fix your connection, or
pre-download the model once, and it will use it from cache thereafter.
