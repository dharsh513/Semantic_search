...
class HybridRetriever:
    ...
    def rollup_documents(
        ...
        """
        top_k = int(top_k or config.TOP_K) if top_k is not None else None
        ...
        for hit in chunk_hits:
            ...
            bucket["best_dense"] = max(bucket["best_dense"], hit.get("dense_score", 0.0))
            bucket["best_lexical"] = max(bucket["best_lexical"], hit.get("lexical_score", 0.0))
        ...
        for pmid, bucket in buckets.items():
            ...
            dense_score = bucket["best_dense"]
            lexical_score = bucket["best_lexical"]
            semantic_pct = int(round(min(1.0, max(0.0, float(dense_score))) * 100))
            keyword_pct = int(round(min(1.0, max(0.0, float(lexical_score))) * 100))
            overall_pct = int(round(min(1.0, max(0.0, float(score))) * 100))

            # Build evidence-grounded 'Why this paper?' explanations
            reasons = []
            if semantic_pct >= 60:
                reasons.append("Strong semantic concept match with research query intent")
            elif semantic_pct >= 35:
                reasons.append("Moderate semantic relevance to topic domain")

            if keyword_pct >= 50:
                reasons.append("High exact keyword overlap in abstract and title")
            elif keyword_pct >= 20:
                reasons.append("Matches key biomedical entities from search query")

            mesh_list = article.get("mesh_terms") or []
            if mesh_list:
                top_mesh_sample = ", ".join(mesh_list[:3])
                reasons.append(f"Indexed under relevant MeSH terms: {top_mesh_sample}")

            top_abstract_passage = next((p for p in passages if (p.get("section") or "").lower() != "title"), None)
            if top_abstract_passage:
                sec_name = top_abstract_passage.get("section") or "Abstract"
                snippet = top_abstract_passage.get("text", "")[:120].strip()
                if snippet:
                    reasons.append(f"Direct evidence in {sec_name}: “{snippet}…”")

            if not reasons:
                reasons.append("Matches candidate literature pool based on PubMed search terms")

            why_this_paper = {
                "semantic_score": round(float(dense_score), 4),
                "bm25_score": round(float(lexical_score), 4),
            ...
            docs.append(
                {
                    ...
                    "semantic_score": round(float(dense_score), 5),
                    "bm25_score": round(float(lexical_score), 5),
                    "hybrid_score": round(float(score), 5),
                    "relevance": round(min(float(score), 1.0) * 100, 1),
                    "why_this_paper": why_this_paper,
                    "matched_passages": [
                        {
                            "section": p.get("section", ""),
                            "text": p["text"],
                            "score": p["score"],
                            "dense_score": p.get("dense_score", 0.0),
                            "lexical_score": p.get("lexical_score", 0.0),
                        }
                        for p in passages
                    ],
                }
