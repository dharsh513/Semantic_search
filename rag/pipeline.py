...
import math
from rag.landscape import landscape_engine
...
class SearchPipeline:
    ...
    def _esearch_with_fallback(
        ...
        for i, term in enumerate(attempts):
            ...
            if res["ids"]:
                ...
                return last
            if not res["ids"] and len(attempts) > 1 and i == 0:
                log.info("Relaxing query '%s' -> '%s'", term, attempts[1])
        return last
    ...
    def search(
        ...
        page_size = int(page_size or top_k or config.TOP_K)
        ...
        if not pmids:
            ...
            payload = {
                ...
                "landscape": landscape_engine.build_landscape([], query=raw_query, total_matches=0),
                "took_ms": took,
            }
            payload["search_id"] = (
                self._record_history(payload, search_field, user_id)
        ...
        chunk_hits = retriever.retrieve(
            query_text, query_vec, top_k_chunks=max(config.TOP_K_CHUNKS, len(articles) * 2)
        )
        all_docs = HybridRetriever.rollup_documents(
            chunk_hits, articles, top_k=len(articles), query_terms=understanding.get("terms")
        )
        ...
        pagination_info = {
            ...
            "total_matches": stages.get("total_matches", total_results),
            "start_index": start_idx + 1 if total_results > 0 else 0,
            "end_index": end_idx,
        }

        # Answer is always built from the top-scoring candidate documents for best quality
        answer_candidates = all_docs[:config.ANSWER_MAX_SOURCES]
        answer = (
            generate(query_text, answer_candidates, query_vec)
            if with_answer
            else {"answer": "", "citations": [], "mode": "skipped", "grounded": False}
        )

        # Build research landscape over the retrieved candidate pool
        landscape_data = (
            landscape_engine.build_landscape(
                list(articles.values()),
                query=raw_query,
                total_matches=stages.get("total_matches", len(articles)),
            )
            if with_landscape
            else None
        )
        ...
        payload = {
            ...
            "results": page_docs,
            "all_pmids": [d["pmid"] for d in all_docs],
    ...
    def landscape(self, query: str, pmids: Optional[List[str]] = None) -> Dict[str, Any]:
        """Direct entry point for research landscape generation."""
        if not pmids:
            # Run quick search to fetch candidate articles
            res = self.search(query, top_k=30, with_answer=False, with_landscape=True, record_history=False)
            return res.get("landscape") or landscape_engine.build_landscape([], query=query)
        
        cached = store.get_articles(pmids)
        missing = [p for p in pmids if p not in cached]
        if missing:
            fetched = self.article_batch(missing)
            cached.update({a["pmid"]: a for a in fetched})
        articles = [cached[p] for p in pmids if p in cached]
        return landscape_engine.build_landscape(articles, query=query, total_matches=len(articles))


    # ------------------------------------------------------------------ #
    @staticmethod
    def _record_history(payload: Dict[str, Any], field: str,
                        user_id: int = 0) -> Optional[int]:
        """
