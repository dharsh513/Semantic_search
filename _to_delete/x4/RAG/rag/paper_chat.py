...
# --------------------------------------------------------------------------- #
def chat(user_id: int, pmid: str, question: str,
         persist: bool = True) -> Dict[str, Any]:
    """
    Answer a question and append both turns to that user's transcript.

    Transcripts are per user: two people reading the same paper keep separate
    conversations.
    """
    article = store.get_articles([pmid]).get(str(pmid))
    ...
    if persist:
        store.add_chat_message(user_id, pmid, "user", question)
        store.add_chat_message(
            user_id, pmid, "assistant", result["answer"],
