...
(function () {
  ...
  if (el.signupForm) {
    el.signupForm.addEventListener("submit", async e => {
      ...
      if (adopted.searches) {
        toast("Adopted " + adopted.searches + " existing search(es) into your account.");
        setBusy(button, true, "Setting up…", "Create account");
        setTimeout(() => go(r.data.next), 1500);
        return;
      }

      setBusy(button, true, "All set…", "Create account");
      go(r.data.next);
    });
  }
