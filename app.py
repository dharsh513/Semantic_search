...
from rag.comparator import comparator
...
def _run_search(force_answer: bool = False):
    ...
    page_size = max(1, min(_int("page_size", _int("top_k", config.TOP_K)), 50))
...
@app.route("/api/landscape", methods=["GET", "POST"])
@login_required
def landscape():
    """Research landscape analysis for a query or specific PMIDs."""
    payload = request.get_json(silent=True) or {}
    args = request.args
    query = (payload.get("query") or args.get("q") or args.get("query") or "").strip()
    pmids = payload.get("pmids") or (args.get("pmids").split(",") if args.get("pmids") else [])
    pmids = [str(p).strip() for p in pmids if str(p).strip()]

    if not query and not pmids:
        return jsonify({"error": "Either 'query' or 'pmids' must be provided."}), 400

    try:
        result = pipeline.landscape(query=query, pmids=pmids)
        return jsonify(result)
    except Exception as exc:  # noqa: BLE001
        log.exception("Landscape analysis failed")
        return jsonify({"error": f"Landscape analysis failed: {exc}"}), 500


@app.post("/api/compare")
@login_required
def compare_papers():
    """Side-by-side structured comparison across 2–5 papers."""
    payload = request.get_json(silent=True) or {}
    pmids = payload.get("pmids") or []
    pmids = [str(p).strip() for p in pmids if str(p).strip()]

    if len(pmids) < 2:
        return jsonify({"error": "Please select at least 2 papers to compare."}), 400
    if len(pmids) > 5:
        return jsonify({"error": "You can compare up to 5 papers at a time."}), 400

    try:
        result = comparator.compare(pmids)
        return jsonify(result)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except PubMedError as exc:
...
@login_required
def research_gaps():
    """AI-assisted research gap identification and suggested directions."""
    payload = request.get_json(silent=True) or {}
    pmids = payload.get("pmids") or []
    pmids = [str(p).strip() for p in pmids if str(p).strip()]

    if not pmids:
        return jsonify({"error": "Please select at least one paper for research gap analysis."}), 400

    try:
        result = gap_engine.analyze_gaps(pmids)
        return jsonify(result)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except PubMedError as exc:
        ...
        log.exception("Research gap analysis failed")
        return jsonify({"error": f"Research gap analysis failed: {exc}"}), 500


@app.get("/api/papers")
@login_required
def papers_list():
    """Retrieve multiple papers or list locally cached articles."""
    pmids_param = request.args.get("pmids", "").strip()
    if pmids_param:
        pmids = [p.strip() for p in pmids_param.split(",") if p.strip()]
        ...
        if missing:
            try:
                ...
                log.warning("Could not fetch missing articles: %s", exc)
        return jsonify({"papers": [cached[p] for p in pmids if p in cached]})

    limit = max(1, min(int(request.args.get("limit", 20)), 100))
    articles = store.all_articles(limit=limit)
    return jsonify({"papers": articles, "total": len(articles)})
